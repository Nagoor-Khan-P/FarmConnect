    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: "Your farmer account has been created successfully. You can now sign in.",
        });
        router.push("/auth/login");
      } else {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || "Failed to create farmer account");
      }
    } catch (error: any) {
      toast({
        variant: "destructive",
      ...
      <main className="flex-grow flex items-center justify-center p-4 py-20 bg-muted/30">
        <Card className="w-full max-w-md shadow-xl border-t-4 border-t-primary">
          <CardHeader className="text-center space-y-2">
            ...
            <CardTitle className="text-2xl font-bold font-headline">Farmer Partnership</CardTitle>
            <CardDescription>Join our community and start selling your yields directly to consumers.</CardDescription>
          </CardHeader>
